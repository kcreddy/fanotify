...
func fileDeleteSelf() (uint, uint64) {
	log.Printf("unix.FAN_CLASS_NOTIF: %v | unix.FD_CLOEXEC: %v | unix.FAN_REPORT_FID: %v", unix.FAN_CLASS_NOTIF, unix.FD_CLOEXEC, unix.FAN_REPORT_FID)
...
func watch(watchDir string) {
	...
	log.Printf("Returned initFlags: %v | markMaskFlags: %v", initFlags, markMaskFlags)
	...
	log.Printf("Returned fd: %v | errno: %v", fd, errno)
	if errno != nil {
		log.Fatalf("FanotifyInit: %v", errno)
	}

	log.Printf("Calling FanotifyMark")
	// fanotify_mark
